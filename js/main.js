	...
	*/
	...
	var resizePizzas = function(size) {
	  ...
	  function changePizzaSizes(size) {
		//Calculate number of pizza elements on the page just one time
		var num_pizzas = document.querySelectorAll(".randomPizzaContainer").length;
		
		//All pizzas are the same width
		//So determine the width of the first pizza element
		var dx = determineDx(document.querySelectorAll(".randomPizzaContainer")[0], size);
		
		//All pizzas will resize to the same width
		//So calculate what the new width should be for the first pizza element
		//We can then use this for all the pizza elements
		var newwidth = (document.querySelectorAll(".randomPizzaContainer")[0].offsetWidth + dx) + 'px';
		
		//Retrieve a list of all pizza HTML elements
		var elements = document.getElementsByClassName("randomPizzaContainer");
	    
	    for (var i = 0; i < num_pizzas; i++) {
	      //Resize each pizza element
